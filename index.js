      ...
      <div class="flex justify-between h-16">
        ...
        <div class="flex items-center space-x-4">
          ...
          <a href="/admin/config" class="text-gray-700 hover:text-gray-900 px-3 py-2 rounded-md text-sm font-medium">
            <i class="fas fa-cog mr-1"></i>系統組態
          </a>
          ...
          <thead class="bg-gray-50">
            <tr>
              ...
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 20%;">
                到期時間 <i class="fas fa-sort-up ml-1 text-indigo-500" title="按到期時間昇冪排列"></i>
              </th>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 15%;">
                提醒設置
              </th>
      ...
      <form id="subscriptionForm" class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="text" id="customType" placeholder="例如：流媒體、雲服務、軟體等"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
            <div class="error-message text-red-500"></div>
          </div>
        </div>
...
<body class="bg-gray-100 min-h-screen">
  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
      <h2 class="text-2xl font-bold text-gray-800 mb-6">系統組態</h2>
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <div id="webhookConfig" class="config-section">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
              ...
              <div>
                <label for="webhookTemplate" class="block text-sm font-medium text-gray-700">消息範本 (JSON格式，可選)</label>
                ...
                <p class="mt-1 text-sm text-gray-500">支援變數: {{title}}, {{content}}, {{timestamp}}。留空使用默認格式</p>
              </div>
            </div>
            <div class="flex justify-end">
          ...
          <div id="emailConfig" class="config-section">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
              ...
              <div>
                <label for="emailFrom" class="block text-sm font-medium text-gray-700">寄件者郵箱</label>
                ...
                <p class="mt-1 text-sm text-gray-500">必須是已在Resend驗證的功能變數名稱郵箱</p>
              </div>
              <div>
                <label for="emailFromName" class="block text-sm font-medium text-gray-700">寄件者名稱</label>
                ...
                <p class="mt-1 text-sm text-gray-500">顯示在郵件中的寄件者名稱</p>
              </div>
              <div>
    ...
    async function testNotification(type) {
      ...
      if (type === 'telegram') {
        ...
        if (!config.RESEND_API_KEY || !config.EMAIL_FROM || !config.EMAIL_TO) {
          showToast('請先填寫 Resend API Key、寄件者郵箱和收件人郵箱', 'warning');
          button.innerHTML = originalContent;
          button.disabled = false;
          return;
        }
...
const admin = {
  async handleRequest(request, env, ctx) {
    try {
      ...
      if (!user) {
        console.log('[管理頁面] 使用者未登錄，重定向到登錄頁面');
        return new Response('', {
          status: 302,
          headers: { 'Location': '/' }
        });
    ...
    if (path.startsWith('/notify/')) {
      ...
      if (method === 'POST') {
        try {
          ...
          // 使用多管道發送通知
          await sendNotificationToAllChannels(title, content, config, '[協力廠商API]');
          ...
          console.error('[協力廠商API] 發送通知失敗:', error);
          return new Response(
            JSON.stringify({
              message: '發送失敗',
              response: {
...
async function testSingleSubscriptionNotification(id, env) {
  try {
    ...
    return { success: true, message: '測試通知已發送到所有啟用的管道' };
...
async function sendWechatBotNotification(title, content, config) {
  try {
    ...
    console.log('[企業微信機器人] 發送消息資料:', JSON.stringify(messageData, null, 2));
    ...
    console.log('[企業微信機器人] 回應狀態:', response.status);
    console.log('[企業微信機器人] 回應內容:', responseText);
    ...
    if (response.ok) {
      try {
        ...
        console.error('[企業微信機器人] 解析回應失敗:', parseError);
        return false;
      }
    } else {
...
async function sendNotificationToAllChannels(title, commonContent, config, logPrefix = '[定時任務]') {
    if (!config.ENABLED_NOTIFIERS || config.ENABLED_NOTIFIERS.length === 0) {
        console.log(`${logPrefix} 未啟用任何通知管道。`);
        return;
    }
...
<body>
  ...
  <div class="info">
    ...
    <p>3. 如果仍有問題，請檢查Cloudflare Workers日誌</p>
        ...
        return new Response(`調試分頁錯誤: ${error.message}`, {
          status: 500,
          headers: { 'Content-Type': 'text/plain; charset=utf-8' }
        });
      }
